...
class DataHandleSVR(DataHandle):  # Inherit from your existing DataHandle class
    ...
    def train_predict_svr(self, train_data, test_data, target_code, additional_vars, best_params):
        ...
        # Train and predict with optimized SVR parameters
    ...
    def SVR_param_selection(self, X, y, nfolds):
        ...
        grid_search = GridSearchCV(SVR(), svr_parameters, scoring='neg_mean_absolute_percentage_error', cv=nfolds)
        grid_search.fit(X, y)
...
best_params = DH.SVR_param_selection(X_full_scaled, y_full.values.ravel(), nfolds)
...
for i in range(60 // offset_month):
    ...
    # Predict using SVR
    predictions, mape = DH.train_predict_svr(train_data, test_data, target_code, additional_vars=vars, best_params=best_params)
